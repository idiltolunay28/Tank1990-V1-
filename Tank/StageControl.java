...
public class StageControl {
    ...
    public void updateStage() {
        ...
        Iterator<Tanks> iterator1 = aliveTanks.iterator();
        while (iterator1.hasNext()) {
            Tanks tank = iterator1.next();
            if (tank.isAlive) {

                boolean p1HasTimer = player1.getBoosters().contains(tempTimer);
                boolean p2HasTimer = false;
                if (player2 != null) {
                    p2HasTimer = player2.getBoosters().contains(tempTimer);
                }

                if (tank.getShield()) {
                    if (time - tank.getShieldStart() >= tank.getShieldDuration()) {
                ...
                if (!p1HasTimer || !p2HasTimer) {
                    if (time - tank.getLastMove() >= tank.getMoveDuration()) {
                        ...
                        tank.setLastMove(Tank1990.getGamePanel().getTime());
                        tank.fire();
                    }

                    for (TankBullet bullet : tank.activeBullets) {
                        if (!bullet.getAlive()) {
                            tank.brokenBullets.add(bullet);
                        } else {
                    ...
                    if (totalTanksSpawned != totalTank) {
                        spawnTank();
                    }
                }
                tank.getActiveBullets().removeAll(tank.getBrokenBullets());
                tank.getBrokenBullets().clear();
            }
        }

        aliveTanks.removeAll(deadTanks);
        deadTanks.clear();

        Iterator<Components> iterator2 = currentMap.iterator();
        while (iterator2.hasNext()) {
            Components comp = iterator2.next();
            if (comp.isDestroyed) {
                destroyedComponents.add(comp);
            }
        }
        if (!destroyedComponents.isEmpty()) {
            currentMap.removeAll(destroyedComponents);
        }
        destroyedComponents.clear();

        checkBoosters();

        if (!claimedBoosters.isEmpty()) {
            unclaimedBoosters.removeAll(claimedBoosters);
            claimedBoosters.clear();
        }
    ...
    public void checkBullet(Bullets bullets) {
        ...
        synchronized (currentMap) {
            for (Components comp : currentMap) {
                if (comp instanceof Destructible && !(comp instanceof Walkable)) {
                    Rectangle forcomp = new Rectangle(comp.getX() * 64, comp.getY() * 64, comp.getSize(), comp.getSize());
